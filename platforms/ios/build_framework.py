...
class Builder:
    ...
    def getBD(self, parent, t):
        ...
        if len(t[0]) == 1:
            res = os.path.join(parent, 'build-%s-%s' %
                               (t[0][0].lower(), t[1].lower()))
        else:
    ...
    def _build(self, outdir):
        ...
        for t in alltargets:
            ...
            if self.contrib:
                cmake_flags.append(
                    "-DOPENCV_EXTRA_MODULES_PATH=%s" % self.contrib)
    ...
    def mergeLibs(self, builddir):
        res = os.path.join(builddir, "lib", "Release", "libbaselib.a")
    ...
    def makeFramework(self, outdir, builddirs):
        ...
        if self.dynamic:
            ...
            libname = "baselib.framework/baselib"
        else:
            dstdir = os.path.join(framework_dir, "Versions", "A")
            libname = "libbaselib.a"

        # copy headers from one of build folders
        shutil.copytree(os.path.join(
            builddirs[0], "install", "include", "opencv2"), os.path.join(dstdir, "Headers"))
        ...
        if self.dynamic:
            resdir = dstdir
            shutil.copyfile(self.getInfoPlist(builddirs),
                            os.path.join(resdir, "Info.plist"))
        else:
            # copy Info.plist
...
class iOSBuilder(Builder):
    ...
    def getToolchain(self, arch, target):
        toolchain = os.path.join(self.opencv, "platforms", "ios",
                                 "cmake", "Toolchains", "Toolchain-%s_Xcode.cmake" % target)
        return toolchain
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='The script builds OpenCV.framework for iOS.')
    parser.add_argument('out', metavar='OUTDIR',
                        help='folder to put built framework')
    parser.add_argument('--baselib', metavar='DIR', default=folder,
                        help='folder with opencv repository (default is "../.." relative to script location)')
    parser.add_argument('--contrib', metavar='DIR', default=None,
                        help='folder with opencv_contrib repository (default is "None" - build only main framework)')
    parser.add_argument('--without', metavar='MODULE', default=[],
                        action='append', help='OpenCV modules to exclude from the framework')
    parser.add_argument('--dynamic', default=False, action='store_true',
                        help='build dynamic framework (default is "False" - builds static framework)')
    parser.add_argument('--disable-bitcode', default=False, dest='bitcodedisabled',
                        action='store_true', help='disable bitcode (enabled by default)')
